...
namespace wms.Client.ViewModel
{
    ...
    public class CheckMainViewModel : DataProcess<CheckMain>
    {
        ...
        /// 盘点任务明细
        /// </summary>
        private readonly ICheckListContract CheckListContract;
        ...
        private readonly IRepository<CheckMain, int> CheckRepository;
        ...
        public CheckMainViewModel()
        {
            ...
            CheckRepository = IocResolver.Resolve<IRepository<CheckMain, int>>();
            CheckContract = IocResolver.Resolve<ICheckContract>();
            CheckListContract = IocResolver.Resolve<ICheckListContract>();
            ...
            SelectItemCommanded = new RelayCommand<object>(SelectInTaskItem, true);
        ...
        private RelayCommand _selectItemCommanded;
        /// <summary>
        /// 选择项目行
        /// </summary>
        public RelayCommand<object> SelectItemCommanded { get; private set; }
        ...
        /// DataGrid列表数据
        /// </summary>
        public ObservableCollection<CheckDetailDto> CheckDetailListTray
        {
            get { return _CheckDetailListTray; }
            set { _CheckDetailListTray = value; RaisePropertyChanged(); }
        }
        private ObservableCollection<CheckDetailDto> _CheckDetailListTray;

        /// <summary>
        /// DataGrid列表-物料条码
        /// </summary>
        public string MaterialLabelTray
        {
            get { return materialLabelTray; }
            set { materialLabelTray = value; RaisePropertyChanged(); }
        }
        private string materialLabelTray;

        /// <summary>
        /// DataGrid列表-所在托盘
        /// </summary>
        public int? TrayCodeTray
        {
            get { return trayCodeTray; }
            set { trayCodeTray = value; RaisePropertyChanged(); }
        }
        private int? trayCodeTray;

        /// <summary>
        /// DataGrid列表-所在储位
        /// </summary>
        public string LocationCodeTray
        {
            get { return locationCodeTray; }
            set { locationCodeTray = value; RaisePropertyChanged(); }
        }
        private string locationCodeTray;

        /// <summary>
        /// DataGrid列表-物料名称
        /// </summary>
        public string MaterialNameTray
        {
            get { return materialNameTray; }
            set { materialNameTray = value; RaisePropertyChanged(); }
        }
        private string materialNameTray;

        /// <summary>
        /// DataGrid列表-物料编码
        /// </summary>
        public string MaterialCodeTray
        {
            get { return materialCodeTray; }
            set { materialCodeTray = value; RaisePropertyChanged(); }
        }
        private string materialCodeTray;

        /// <summary>
        /// DataGrid列表-盘点数量
        /// </summary>
        public decimal? CheckedQuantityTray
        {
            get { return checkedQuantityTray; }
            set { checkedQuantityTray = value; RaisePropertyChanged(); }
        }
        private decimal? checkedQuantityTray;
        ...
        public async void SelectInTaskItem(object obj)
        {
            try
            {
                ...
                Bussiness.Dtos.CheckDetailDto entity = obj as Bussiness.Dtos.CheckDetailDto;
                //if (multiObj[1] != null)
                //{
                ...
                GlobalData.GuideCode = entity.CheckCode;
                SelectTrayCode = entity.TrayId.ToString();
        ...
        public async void SelectTray()
        {
            ...
            {
                // 获取当前任务的明细
                var ListEntity = CheckContract.CheckDetailDtos.Where(a => a.TrayCode == SelectTrayCode && a.Status == 0).ToList();
                CheckDetailListTray = new ObservableCollection<CheckDetailDto>(ListEntity);

                //foreach (var checkListDetail in ListEntity)
                //{
                //    var material = MaterialContract.Materials.FirstOrDefault(a => a.Code == checkListDetail.MaterialCode);
                //    MaterialNameTray = material?.Name; // 如果找到了物料，将物料名称赋值给MaterialName；否则为null
        ...
        public async void HandShelf()
        {
            ...
            try
            {
                ...
                //outLabel.Quantity = (decimal)CheckQuantity;
                ...
                if (inTaskPost.Result.Success)
                {
                    ...
                    // 1. 从列表中找到要删除的行
                    var rowToRemove = CheckDetailListTray.FirstOrDefault(row => row.LocationCode == SearchLocationCode && row.MaterialCode == SelectMaterialCode);

                    if (rowToRemove != null)
                    {
                        // 2. 从数据源中移除该行
                        CheckDetailListTray.Remove(rowToRemove);
                        // 3. 刷新DataGrid以反映更改
                        RaisePropertyChanged(nameof(CheckDetailListTray));
                    }
        ...
        public async void Submit()
        {
            try
            {
                ...
                var OutTaskListes = CheckContract.CheckDetailDtos.Where(a => a.CheckCode == GlobalData.GuideCode && a.Status == 0).ToList();

                // 仍有未完成的任务
                if (OutTaskListes.Count > 0)
                {
                    Msg.Warning("仍有未盘点的物料，请完成盘点");
                    return;
                }
                ...
                var submitEntity = CheckRepository.Query().FirstOrDefault(a => a.Code == GlobalData.GuideCode);

                var submitTastPost = checkService.PostDoSubmit(submitEntity);
                if (submitTastPost.Result.Success)
                {
                    Msg.Info("盘点完成，结果已提交");
                }
